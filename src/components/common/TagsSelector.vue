<template>
  <view class="tags-selector">
    <view class="tags-selector-tags"
      v-if="model.length > 0">
      <nut-space wrap>
        <nut-tag v-for="(tag, index) in model"
          type="primary" :key="index" closeable
          @close="removeTag(tag)">
          {{ tag }}
        </nut-tag>
      </nut-space>
    </view>
    <view class="tags-selector-input"
      v-if="tagsNotFull || tagsNoLimit">
      <custom-wrapper>
        <nut-input v-model="tagName"
          :placeholder="`标签最多${props.tagMaxLength}字`"
          type="text" :max-length="props.tagMaxLength" />
      </custom-wrapper>
      <nut-button class="tags-selector-input-add"
        type="primary" shape="round" size="small"
        @click="addTag" :disabled="!enableAddTag"
        :style="{ flex: 'none' }">添加</nut-button>
    </view>
  </view>
</template>
